...
def test_xor_decrypt():
    ...
    for key in keys:
        ...
        for decrypted, used_key, percent in xornlpcrack.xor_decrypt_guess_key(encrypted, len(key)):
            # Check if decryption is successful and matches the original plaintext
            if decrypted == plaintext:
                print(f"key '{used_key}' for '{decrypted}' {percent*100}% match, took {round(time.time() - start_time, 3)} sec")
                break
